...
export class ProjectFilesProvider implements vscode.TreeDataProvider<ProjectItem> {
    ...
    private getRawCheckboxState(
        state: vscode.TreeItemCheckboxState | { state: vscode.TreeItemCheckboxState; tooltip?: string; accessibilityInformation?: vscode.AccessibilityInformation } | undefined
    ...
    getTreeItem(element: ProjectItem): vscode.TreeItem {
        ...
        if (element.lastCheckboxState === undefined) {
            ...
            if (element.lastCheckboxState === vscode.TreeItemCheckboxState.Unchecked && currentState === vscode.TreeItemCheckboxState.Unchecked) {
                this.selectedItems.add(fsPath);
                element.checkboxState = vscode.TreeItemCheckboxState.Checked;
            } else if (element.lastCheckboxState === vscode.TreeItemCheckboxState.Checked && currentState === vscode.TreeItemCheckboxState.Checked) {
                this.selectedItems.delete(fsPath);
                element.checkboxState = vscode.TreeItemCheckboxState.Unchecked;
            }
            element.lastCheckboxState = this.getRawCheckboxState(element.checkboxState);
        }
        return element;
    }
    ...
    async getChildren(element?: ProjectItem): Promise<ProjectItem[]> {
        ...
        try {
            ...
            if (!element) {
                vscode.window.showErrorMessage(`Failed to read workspace directory: ${error}`);
            }
            return [];
        }
    }
